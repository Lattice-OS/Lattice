--- This Sha2 implementation uses code which is licensed under the MIT License.
--- The original code can be found at https://github.com/somesocks/lua-lockbox/
---
--- This borrowed code is modified to remove the module dependencies which are not explicitly required for it to work.
--- It also inlines the modules which are entirely required for them to work.
---
--- If my theory is correct, this code should be entirely compatible with all versions of ComputerCraft
--- and should provided a consistent hash across all of them.

function lockbox_bit()
    local ok, e
    ok = nil
    if not ok then
...
local Bit         = lockbox_bit();
local String      = require("string");
local Math        = require("math");
local Queue       = lockbox_queue();
